...
import { ExtensionContext, workspace, commands, window, ViewColumn, Uri, WebviewPanel, Position, Selection, Range, env } from 'vscode';
import {
  LanguageClient,
...
import { tokenize, toHtml } from './highlighter';

let client: LanguageClient;
let hierarchyPanel: WebviewPanel | undefined;
let presentationPanel: WebviewPanel | undefined;
...
export function activate(context: ExtensionContext): void {
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    synchronize: {
      fileEvents: workspace.createFileSystemWatcher('**/*.{sv,svh,svi,v,vh,vl}'),
    },
  };
  ...
  const hierarchyCmd = commands.registerCommand('svLsp.showClassHierarchy', async () => {
    ...
    if (!wordRange) {
      window.showWarningMessage('Place your cursor on a class name');
      return;
    }
    ...
    if (!client.isRunning()) {
      window.showWarningMessage('Language server is not running yet. Please wait a moment.');
      return;
    }

    const hierarchy: HierarchyNode | null = await client.sendRequest('svLsp/classHierarchy', { className });

    if (!hierarchy) {
      window.showInformationMessage(`No class hierarchy found for "${className}"`);
      return;
    }
  ...
  const doFileCmd = commands.registerCommand('svLsp.generateDoFile', async () => {
    const editor = window.activeTextEditor;
    if (!editor) {
      window.showWarningMessage('No active editor');
      return;
    ...
    const result: DependencyChainResult | null = await client.sendRequest('svLsp/generateDoFile', { uri: fileUri });

    if (!result || result.files.length === 0) {
      window.showWarningMessage('Could not determine dependencies for this file.');
      return;
    }
    ...
    const defaultUri = Uri.file(path.join(currentDir, 'sim.do'));
    ...
    if (saveUri) {
      ...
      window.showInformationMessage(`DO file saved to ${saveUri.fsPath}`);
  ...
  const copyPresCmd = commands.registerCommand('svLsp.copyForPresentation', () => {
    const editor = window.activeTextEditor;
    if (!editor) {
      window.showWarningMessage('No active editor');
      return;
    ...
    if (!code.trim()) {
      window.showWarningMessage('No code to copy');
      return;
    }

    const config = workspace.getConfiguration('svLsp');
    const fontFamily: string = config.get('presentation.fontFamily', "Consolas, 'Courier New', monospace");
    const fontSize: number = config.get('presentation.fontSize', 14);
    const theme: 'dark' | 'light' = config.get('presentation.theme', 'dark');
  ...
  let questaTerminal: import('vscode').Terminal | undefined;

  const runQuestaCmd = commands.registerCommand('svLsp.runQuestaSim', async () => {
    const editor = window.activeTextEditor;
    const defaultDir = editor
      ? Uri.file(path.dirname(editor.document.uri.fsPath))
      : workspace.workspaceFolders?.[0]?.uri;

    const doFileUris = await window.showOpenDialog({
      canSelectMany: false,
    ...
    const config = workspace.getConfiguration('svLsp');
    const vsimPath: string = config.get('questaSimPath', 'vsim');
    ...
    questaTerminal = window.createTerminal({ name: 'QuestaSim', cwd: doFileDir });
    questaTerminal.show();
...
function getDefaultShellPath(): string {
  ...
  const profiles: Record<string, { path?: string }> | undefined =
    workspace.getConfiguration('terminal.integrated.profiles').get(platformKey);
  const defaultName: string | null | undefined =
    workspace.getConfiguration('terminal.integrated').get(`defaultProfile.${platformKey}`);
...
function showPresentationPanel(
  code: string,
  fontFamily: string,
  fontSize: number,
  theme: 'dark' | 'light',
  ...
  if (presentationPanel) {
    presentationPanel.reveal(ViewColumn.Beside);
  } else {
    presentationPanel = window.createWebviewPanel(
      'svCodePresentation',
      'SV Code Preview',
      ViewColumn.Beside,
      { enableScripts: true },
  ...
  presentationPanel.webview.onDidReceiveMessage((message) => {
    if (message.command === 'copied') {
      window.showInformationMessage('Code copied with syntax highlighting! Paste into PowerPoint.');
    }
  });

  const darkHtml = toHtml(tokens, 'dark', fontFamily, fontSize);
  const lightHtml = toHtml(tokens, 'light', fontFamily, fontSize);
  const initialTheme = theme;

  presentationPanel.webview.html = buildPresentationHtml(
    darkHtml,
    lightHtml,
    initialTheme,
    fontSize,
...
function showHierarchyPanel(root: HierarchyNode, context: ExtensionContext): void {
  if (hierarchyPanel) {
    hierarchyPanel.reveal(ViewColumn.Beside);
  } else {
    hierarchyPanel = window.createWebviewPanel(
      'svClassHierarchy',
      'SV Class Hierarchy',
      ViewColumn.Beside,
      {
  ...
  hierarchyPanel.webview.onDidReceiveMessage(async (message) => {
    if (message.command === 'openFile' && message.uri && message.line >= 0) {
      const fileUri = Uri.parse(message.uri);
      const doc = await workspace.openTextDocument(fileUri);
      const editor = await window.showTextDocument(doc, ViewColumn.One);
      const pos = new Position(message.line, 0);
...
function buildHierarchyHtml(root: HierarchyNode): string {
...
function renderNode(node: HierarchyNode, isRoot: boolean): string {
...
function toSimPath(fsPath: string): string {
  if (process.platform === 'win32') {
...
function formatDoFile(result: DependencyChainResult, sourceUri: Uri): string {
  ...
  const incDirPaths = new Set<string>();
  for (const dirUri of result.includeDirs) {
    const parsed = Uri.parse(dirUri);
  ...
  for (const file of result.files) {
    const parsed = Uri.parse(file.uri);
