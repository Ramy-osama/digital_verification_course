...
exports.getDotContext = getDotContext;
...
(function (SymbolKind) {
    ...
    SymbolKind["Covergroup"] = "covergroup";
 ...
 */
function stripComments(text, blankStrings = false) {
    let result = '';
    let i = 0;
...
function parseDocument(text) {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        if (portParsedLines.has(lineIdx)) {
            const paramInPortMatch = line.match(/^parameter\s+((?:(?:logic|reg|wire|bit|int|integer|real|signed|unsigned)\s*(?:\[[^\]]*\]\s*)?)?)\s*([a-zA-Z_][a-zA-Z0-9_$]*)\s*=/);
            if (paramInPortMatch) {
        ...
        const scopeOpenMatch = line.match(/^(?:virtual\s+)?(module|macromodule|class|function|task|interface|package|program|generate|covergroup)\s+(?:(?:automatic|static)\s+)?(?:(?:void|int|logic|bit|byte|shortint|longint|integer|string|real|realtime|time)\s*(?:\[[^\]]*\]\s*)?)?([a-zA-Z_][a-zA-Z0-9_$]*)?/);
        if (scopeOpenMatch) {
            const keyword = scopeOpenMatch[1];
            const name = scopeOpenMatch[2] || `<anonymous_${lineIdx}>`;
            const scopeKind = svLanguage_1.SCOPE_OPEN_KEYWORDS[keyword] || svLanguage_1.ScopeKind.Begin;
            const col = rawLine.indexOf(name);
            let symbolKind;
            switch (scopeKind) {
                case svLanguage_1.ScopeKind.Module:
                    symbolKind = SymbolKind.Module;
                    break;
                case svLanguage_1.ScopeKind.Class:
                    symbolKind = SymbolKind.Class;
                    break;
                case svLanguage_1.ScopeKind.Function:
                    symbolKind = SymbolKind.Function;
                    break;
                case svLanguage_1.ScopeKind.Task:
                    symbolKind = SymbolKind.Task;
                    break;
                case svLanguage_1.ScopeKind.Interface:
                    symbolKind = SymbolKind.Interface;
                    break;
                case svLanguage_1.ScopeKind.Package:
                    symbolKind = SymbolKind.Package;
                    break;
                case svLanguage_1.ScopeKind.Program:
                    symbolKind = SymbolKind.Program;
                    break;
                case svLanguage_1.ScopeKind.Covergroup:
                    symbolKind = SymbolKind.Covergroup;
                    break;
            ...
            if (scopeKind === svLanguage_1.ScopeKind.Class) {
            ...
            if (scopeKind === svLanguage_1.ScopeKind.Module || scopeKind === svLanguage_1.ScopeKind.Interface) {
                parsePortList(lines, lineIdx, symbols, references, declaredNames, name, portParsedLines);
            }
            continue;
        }
        // --- Scope-closing constructs ---
        const scopeCloseMatch = line.match(/^(endmodule|endclass|endfunction|endtask|endinterface|endpackage|endprogram|endgenerate|endgroup)\b/);
        if (scopeCloseMatch) {
        ...
        if (/^begin\b/.test(line)) {
            ...
            scopeStack.push({
                kind: svLanguage_1.ScopeKind.Begin,
        ...
        if (/\bend\b/.test(line) && !/\bendmodule|endclass|endfunction|endtask|endinterface|endpackage|endprogram|endgenerate|endgroup\b/.test(line)) {
            if (scopeStack.length > 0 && scopeStack[scopeStack.length - 1].kind === svLanguage_1.ScopeKind.Begin) {
                scopeStack.pop();
            }
        }
        ...
        const paramMatch = line.match(/^(parameter|localparam)\s+((?:(?:logic|reg|wire|bit|int|integer|real|signed|unsigned)\s*(?:\[[^\]]*\]\s*)?)?)\s*([a-zA-Z_][a-zA-Z0-9_$]*)\s*=/);
        if (paramMatch) {
        ...
        const structOpenMatch = line.match(/^(typedef\s+)?struct\s*(?:packed\s*)?(?:signed\s*|unsigned\s*)?\s*\{/);
        if (structOpenMatch) {
            const isTypedef = !!structOpenMatch[1];
            const structResult = parseStructBody(lines, lineIdx, isTypedef, currentScope(), symbols, references, declaredNames);
            if (structResult) {
        ...
        const typedefMatch = line.match(/^typedef\s+(.+?)\s+([a-zA-Z_][a-zA-Z0-9_$]*)\s*;/);
        if (typedefMatch) {
        ...
        // --- Constraint blocks ---
        const constraintMatch = line.match(/^constraint\s+([a-zA-Z_][a-zA-Z0-9_$]*)\s*\{/);
        if (constraintMatch) {
        ...
        const declMatch = line.match(/^((?:(?:rand|randc|const|static|automatic|local|protected|virtual|var)\s+)*)?(input|output|inout)?\s*(logic|reg|wire|bit|int|integer|real|realtime|time|shortint|longint|byte|shortreal|string|chandle|event|void|genvar|tri|wand|wor|enum|struct)\s*((?:signed|unsigned|packed)\s*)*((?:\[[^\]]*\]\s*)*)\s*(.+)/);
        if (declMatch) {
        ...
        const portOnlyMatch = line.match(/^(input|output|inout)\s+((?:\[[^\]]*\]\s*)*)\s*(.+)/);
        if (portOnlyMatch) {
        ...
        const classVarMatch = line.match(/^((?:(?:rand|randc|const|static|automatic|local|protected|virtual|var)\s+)*)([a-zA-Z_][a-zA-Z0-9_$]*)\s+(.*)/);
        if (classVarMatch) {
            const cvModifiers = (classVarMatch[1] || '').trim();
            const cvTypeName = classVarMatch[2];
            let cvRest = classVarMatch[3].trim();
            if (!svLanguage_1.SV_KEYWORDS.has(cvTypeName) && !svLanguage_1.SV_TYPES.has(cvTypeName) &&
                !svLanguage_1.SV_PORT_DIRECTIONS.has(cvTypeName) && !svLanguage_1.SV_STORAGE_MODIFIERS.has(cvTypeName)) {
                ...
                if (cvRest.startsWith('#(')) {
                    ...
                    for (let i = 0; i < cvRest.length; i++) {
                        if (cvRest[i] === '(')
                            ...
                            depth--;
                            if (depth === 0) {
                                cvRest = cvRest.substring(i + 1).trim();
                                break;
                            }
                        }
                    }
 ...
 */
function parseStructBody(lines, startLine, isTypedef, parentScope, symbols, references, declaredNames) {
    let braceDepth = 0;
    ...
    const nameMatch = closingLine.match(/\}\s*([a-zA-Z_][a-zA-Z0-9_$]*)\s*((?:\[[^\]]*\]\s*)*)\s*;/);
    if (!nameMatch)
        return null;
 ...
 */
function parsePortList(lines, startLine, symbols, references, declaredNames, scopeName, portParsedLines) {
    let parenDepth = 0;
    let foundOpen = false;
    let portText = '';
...
function parsePortDeclarations(portText, startLine, allLines, symbols, declaredNames, scopeName) {
    ...
    for (const part of parts) {
        ...
        const portMatch = trimmed.match(/^(input|output|inout)?\s*((?:logic|reg|wire|bit|int|integer|real|signed|unsigned)\s*(?:\[[^\]]*\]\s*)*)?\s*([a-zA-Z_][a-zA-Z0-9_$]*)\s*(?:=\s*[^,]*)?$/);
        if (portMatch) {
            if (portMatch[1])
                currentDirection = portMatch[1];
            if (portMatch[2] && portMatch[2].trim())
                currentType = portMatch[2].trim();
...
function findNameInLines(lines, name, startLine) {
 ...
 */
function parseVariableNames(rest, fullType, kind, lineIdx, rawLine, symbols, references, declaredNames, scope) {
    ...
    for (const part of parts) {
        ...
        if (nameMatch) {
            ...
            if (svLanguage_1.SV_KEYWORDS.has(varName) || svLanguage_1.SV_TYPES.has(varName))
                continue;
 ...
 */
function extractReferencesFromLine(rawLine, lineIdx, references, declaredNames, scope) {
    const identRe = /\b([a-zA-Z_][a-zA-Z0-9_$]*)\b/g;
    let match;
    while ((match = identRe.exec(rawLine)) !== null) {
        const name = match[1];
        if (svLanguage_1.SV_KEYWORDS.has(name) || svLanguage_1.SV_TYPES.has(name) || svLanguage_1.SV_STORAGE_MODIFIERS.has(name))
            continue;
        if (svLanguage_1.SV_PORT_DIRECTIONS.has(name))
            continue;
        if (name.startsWith('$'))
            continue;
        if (/^[0-9]/.test(name))
 ...
 */
function getDotContext(line, character) {
    let start = character;
    while (start > 0 && /[a-zA-Z0-9_$]/.test(line[start - 1]))
        start--;
    ...
    let objEnd = start - 1;
    let objStart = objEnd;
    while (objStart > 0 && /[a-zA-Z0-9_$]/.test(line[objStart - 1]))
        objStart--;
    const objName = line.substring(objStart, objEnd);
    if (!objName || /^[0-9]/.test(objName))
        return null;
    return { objName };
}
